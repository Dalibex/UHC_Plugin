...
public class SetPvpEpisodeCommand implements CommandExecutor, TabCompleter {
    ...
    public SetPvpEpisodeCommand(UHC_DBasic plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length != 1) {
            sender.sendMessage(plugin.getLang().get("pvp.episode.usage", null)
        ...
        if (!isValidEpisode(episode)) {
            sender.sendMessage(plugin.getLang().get("pvp.episode.range", null)
                    .replace("%error-prefix%", plugin.getLang().get("general.error-prefix", null)));
            return true;
        }
        ...
        sender.sendMessage(plugin.getLang().get("pvp.episode.success", null)
